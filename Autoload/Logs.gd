...
func _log_entity_deal_critical_damage(target_type: String, entity_name: String): #on appelle cette fonction quand une entité inflige un coup critique
	if target_type == "Player":
		...
		Logs._add_log("Vous a perdez " +  str(max(1,GameData.player_MT - GameData.enemy_stats[EntitiesState.enemy_id].DEF)*2) + " dégâts")
	elif target_type == "Enemy":
		Logs._add_log("SMAAAASH!!")
		Logs._add_log("vous avez infligé\n" + str(max(1,GameData.enemy_stats[EntitiesState.enemy_id].MT - GameData.player_DEF)*2) + " dégâts")
